...
#TODO what is going on here ... running macs?
```{bash}
...
#TODO not sure what this all is ChiPr peaks?
...
# merging in needed data to samples for Deseq
samples <- samples %>% unite(itg_timepoint, firre_itg, timepoint, remove = FALSE)

#TODO
# setting samples to control?
samples$condition <- "control"

# setting inducd col values 
...
#TODO why -log10 space?
# plotting mean vs fold change
...
# Now testing each geneotype for sig peak differentces
...
#TODO I am guessing the plot has more - values because KO has less K27me3
#TODO investigate bigWig
...
# THere are some sig peaks (spoiler) so filter to 2 fold change (2 to -2)
res <- results(dds, name = "firre_ko_WT_vs_KO")
res_df <- res %>% as.data.frame() %>%
...
# Some sig peaks finally :) 
# TODO make a df of these peaks
sig_threshold <- sig[filter(sig$log2FoldChange >= 1)]
