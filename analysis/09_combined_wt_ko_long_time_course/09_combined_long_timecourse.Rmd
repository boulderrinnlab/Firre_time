...
stopifnot(all(rownames(combined_long_all_timepoints_vszero_counts) == genes$gene_id))
 ...
 # setting result names object to be filtered below
  ...
  # saving 
  save(combined_long_all_timepoints_vszero_shrnklfc, file = "results/combined_vs_zero_all_time_points.RData")
}
...
rescued_gene_ids_20 <- read.csv("../04_Firre_ko_long_time_course/results/rescued_sig_20.txt")

combined_long_sig_genes <- unique(combined_long_vszero_sig$gene_id) 

test <- rescued_gene_ids_20 %in% combined_long_sig_genes

rescued_gene_ids_20 <- rescued_gene_ids_20[rescued_gene_ids_20 %in% combined_long_sig_genes]
rownames_to_column(rescued_gene_ids_20)


combined_sig_genes <- unique(short_vszero_sig$gene_id)
# overlapping %in% combined_sig_genes and rescued 20% WT KO levels 
rescued_ids_20[rescued_ids_20 %in% combined_sig_genes$gene_id]
...
```
### Result: 6 Genes are rescued in long time course ....
